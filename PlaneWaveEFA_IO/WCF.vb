...
Public Class cDB_ServiceContract : Implements IDB
    ...
    '''<summary>DB objec to get information from.</summary>
    Private Shared PWIValues As cPWIValues
    ...
    Public Sub New(ByRef DB_ToUse As cDB, ByRef PWIValues_ToUse As cPWIValues)
        DB = DB_ToUse
        PWIValues = PWIValues_ToUse
    End Sub
    ...
    Public Function GetParameterList() As List(Of String) Implements IDB.GetParameterList
        ...
        Next SingleProperty
        For Each SingleProperty As Reflection.PropertyInfo In PWIValues.GetType.GetProperties()
            RetVal.Add(SingleProperty.Name)
        Next SingleProperty
        RetVal.Sort()
        Return RetVal
    ...
    Public Function GetParameter(Key As String) As Object Implements IDB.GetParameter
        ...
        For Each SingleProperty As Reflection.PropertyInfo In PWIValues.GetType.GetProperties()
            If Key.ToUpper = SingleProperty.Name.ToUpper Then
                Return SingleProperty.GetValue(PWIValues, Nothing)
            End If
        Next SingleProperty
        Return Nothing
    End Function
        ...
        For Each SingleProperty As Reflection.PropertyInfo In PWIValues.GetType.GetProperties()
            If Key.ToUpper = SingleProperty.Name.ToUpper Then
                Select Case SingleProperty.PropertyType.Name.ToUpper
                    Case "Double".ToUpper
                        Dim NewProp As Double
                        If Double.TryParse(Value, NewProp) = True Then SingleProperty.SetValue(PWIValues, NewProp, Nothing)
                End Select
                RaiseEvent ValueChanged()
                Return SingleProperty.GetValue(PWIValues, Nothing)
            End If
